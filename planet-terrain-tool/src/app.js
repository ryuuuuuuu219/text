...
function inspect(index){selected=index;const cell=sim.cells[index],{q,r}=sim.coords(index);$("emptyInspector").hidden=true;$("cellInspector").hidden=false;$("cellTitle").textContent=`CELL ${String(q).padStart(2,"0")}—${String(r).padStart(2,"0")}`;
  if(!cell.crust){$("cellSubtitle").textContent="地殻の空隙";["surfaceValue","thicknessValue","baseValue","densityValue","ageValue","temperatureValue","cellPlateValue","vectorValue"].forEach(id=>$(id).textContent="—");$("directionValue").textContent="継承なし";draw();return}
  const c=cell.crust,e=sim.surfaceElevation(c),d=c.nextDirection;$("cellSubtitle").textContent=e>=sim.seaLevel?"陸域 / 地殻あり":"海域 / 地殻あり";$("surfaceValue").textContent=`${e.toFixed(2)} km`;$("thicknessValue").textContent=`${c.thickness.toFixed(1)} km`;$("baseValue").textContent=`${c.basement.toFixed(1)} km`;$("densityValue").textContent=`${c.density.toFixed(2)} g/cm³`;$("ageValue").textContent=`${Math.round(c.age)} Myr`;$("temperatureValue").textContent=`${Math.round(c.temperature)} °C`;$("cellPlateValue").textContent=`P-${String(cell.plateId).padStart(2,"0")}`;$("vectorValue").textContent=`${c.velocity.x.toFixed(2)}, ${c.velocity.y.toFixed(2)}`;$("directionArrow").style.transform=`rotate(${d*60}deg)`;$("directionValue").textContent=`継承方向 ${DIRECTIONS[d].name}`;$("heightBar").style.width=`${Math.max(4,Math.min(100,(e+12)/28*100))}%`;draw();}
...
$("newWorldButton").onclick=()=>{setRunning(false);sim.reset(Date.now());selected=null;draw()};
...
$("speedInput").oninput=e=>$("speedOutput").textContent=`${e.target.value} steps/s`;$("seaInput").oninput=e=>{sim.seaLevel=Number(e.target.value);$("seaOutput").textContent=`${sim.seaLevel.toFixed(1)} km`;draw()};$("volcanicInput").oninput=e=>{sim.volcanicStrength=Number(e.target.value);$("volcanicOutput").textContent=`${sim.volcanicStrength.toFixed(2)}×`};$("boundaryInput").oninput=e=>{sim.boundaryInfluence=Number(e.target.value);$("boundaryOutput").textContent=sim.boundaryInfluence.toFixed(2)};
...
$("loadInput").onchange=async e=>{try{sim=CrustSimulation.deserialize(await e.target.files[0].text());selected=null;$("seaInput").value=sim.seaLevel;$("volcanicInput").value=sim.volcanicStrength;$("boundaryInput").value=sim.boundaryInfluence;resize()}catch(error){alert(error.message)}e.target.value=""};
